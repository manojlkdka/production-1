...
expires_in = "customer_support_branch_api.customer_support_branch_access_token_expires_in"
...
def validate_token(func):
    ...
    def wrap(self, *args, **kwargs):
        ...
        access_token_data = (
            request.env["customer_support_branch.access_token"].sudo().search([("token", "=", access_token)], order="id DESC", limit=1)
        )
...
class Dn_api(http.Controller):
	def __init__(self):
		self._token = request.env["customer_support_branch.access_token"]
	...
	def token(self, **post):
		_token = request.env["customer_support_branch.access_token"]
	...
	@validate_token
	@http.route("/api/update_support_ticket", type="json", auth="public", methods=["POST"], csrf=False)
	def post_je(self, model=None, id=None, **data):
		...
		model = 'customer.support.branch.api'
		# ioc_name = model
		# model = request.env[self._model].search([("model", "=", model)], limit=1)
		...
		try:
			...
			if payload['client_ticket_id']:
				entries = request.env['customer.support.branch.api'].search([('id','=',payload['client_ticket_id'])])
			if entries:
				entries.write({
	...
	# 	try:
	# 		if 'client_ticket_id' in payload and payload['client_ticket_id']:
	# 			entries = request.env[model].browse(payload['client_ticket_id'])
	# 			if entries:
	# 				if 'response_message' in payload and payload['response_message']:
	# 					entries.response_message = payload['response_message']
