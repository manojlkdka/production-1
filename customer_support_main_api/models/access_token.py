...
class APIAccessToken(models.Model):
    _name = "customer_support_main.access_token"
    ...
    def find_one_or_create_token(self, user_id=None, create=False):
        ...
        access_token = self.env["customer_support_main.access_token"].sudo().search([("user_id", "=", user_id)], order="id DESC", limit=1)
        if access_token:
        ...
        if not access_token and create:
            ...
            access_token = self.env["customer_support_main.access_token"].sudo().create(vals)
        if not access_token:
            return None
...
class Users(models.Model):
    _inherit = "res.users"
    token_ids = fields.One2many("customer_support_main.access_token", "user_id", string="Access Tokens")
